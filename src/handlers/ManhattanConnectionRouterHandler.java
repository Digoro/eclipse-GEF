...
public class ManhattanConnectionRouterHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		for (int i = 0; i < ((MyGraphicalEditor) editor).getGraph().getConnections()
				.size(); i++) {
			Connection gf = ((GraphConnection) ((MyGraphicalEditor) editor)
					.getGraph().getConnections().get(i)).getConnectionFigure();
